...
async function runDeepDiagnostic() {
    ...
    try {
        const response = await fetch('http://localhost:3000/api/chat', {
            ...
            body: JSON.stringify({
                message: `${context}
                Identify 3 things in Russian, returning them as a JSON object with keys: "scenario" (typical procrastination), "optimization" (habit recommendations), and "risks" (forecasting risky periods).
...
async function analyzeProcrastination() {
    ...
    try {
        const response = await fetch('http://localhost:3000/api/chat', {
            ...
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                message: `Analyze productivity for ${done}/${total} tasks. Identify success patterns or risk of failure in Russian. Keep it to 2 insightful sentences.`,
                mode: 'analytical'
            })
        });
        const data = await response.json();
