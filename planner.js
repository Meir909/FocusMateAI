...
async function adaptPlanWithAI() {
    ...
    if (todo.length === 0) {
        createNotification("Adaptive AI", "Список To Do пуст, адаптировать нечего.", "😴");
        return;
    }

    createNotification("Adaptive AI", "Анализирую ваш прогресс для корректировки плана...", "🧠");
    ...
    try {
        ...
        createNotification("Success", "План успешно скорректирован под вашу скорость.", "✅");

    } catch (e) {
        console.error("Adaptation error:", e);
        createNotification("AI Error", "Не удалось адаптировать план автоматически.", "❌");
    }
}
